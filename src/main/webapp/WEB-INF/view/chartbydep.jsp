            ...
            <div>
                <table class="table table-striped">
                    ...
                    <c:if test="${where=='byYear'}">
                        <c:forEach items="${exVoEmp}" var="voEmp" varStatus="i">
                            <tr>
                                <td>${voEmp.department.departmentId}</td>
                                <td>${voEmp.department.departmentName}</td>
                                <td>${voEmp.totalMoney}元</td>
                                <td>${voEmp.year}年</td>
                            </tr>
...
<script>
    ...
    function initPieChart() {
        ...
        //created  表示页面加载完毕，立即执行
        <c:forEach items="${exVoEmp}" var="dx">
            names.push("${dx.department.departmentName}")
            nums.push(${dx.totalMoney})
        </c:forEach>
        option = {
            title : {
                <c:if test="${where=='byMonth'}">
                    text: '${exVoEmp.get(0).year}年${exVoEmp.get(0).month}月月度报销统计图',
                </c:if>
                <c:if test="${where=='byYear'}">
                    text: '${exVoEmp.get(0).year}年度报销统计图',
                </c:if>
            ...
            series : [
                {
                    ...
                    data:[
                        <c:forEach items="${exVoEmp}" var="dx">
                        {value:${dx.totalMoney}, name:'${dx.department.departmentName}'},
                        </c:forEach>
                    ],
                    itemStyle: {
                        emphasis: {
    ...
    function initOtherChart() {
        ...
        option = {
            title : {
                ...
                <c:if test="${where=='byYear'}">
                text: '${exVoEmp.get(0).year}年年度报销统计图',
                </c:if>
            },
            tooltip : {
                trigger: 'axis'
